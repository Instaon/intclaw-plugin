...
if [ -f "$CONFIG_FILE" ]; then
    ...
    # 自动迁移配置（如果有 jq）
    if [ "$HAS_JQ" = true ]; then
        # 先检查当前配置中是否已有 intclaw-connector 配置
        EXISTING_CONNECTOR_ID=$(jq -r '.channels."intclaw-connector".clientId // empty' "$CONFIG_FILE")
        EXISTING_CONNECTOR_SECRET=$(jq -r '.channels."intclaw-connector".clientSecret // empty' "$CONFIG_FILE")
        EXISTING_CONNECTOR_ACCOUNTS=$(jq -r '.channels."intclaw-connector".accounts // empty' "$CONFIG_FILE")
        
        # 如果已经有 intclaw-connector 配置，跳过迁移
        if [ -n "$EXISTING_CONNECTOR_ID" ] || [ -n "$EXISTING_CONNECTOR_SECRET" ] || { [ "$EXISTING_CONNECTOR_ACCOUNTS" != "null" ] && [ -n "$EXISTING_CONNECTOR_ACCOUNTS" ]; }; then
            echo "✅ 检测到已有 intclaw-connector 配置，跳过迁移"
            echo "   - 配置文件保持不变"
        else
            echo "🔄 从原始配置中提取并迁移..."
            
            # 从源配置中提取旧的 intclaw 配置
            OLD_CLIENT_ID=$(jq -r '.channels.intclaw.clientId // empty' "$SOURCE_CONFIG")
            
            # 检查 clientSecret 的类型（字符串或对象）
            SECRET_TYPE=$(jq -r '.channels.intclaw.clientSecret | type' "$SOURCE_CONFIG")
            
            if [ "$SECRET_TYPE" = "string" ]; then
                # 如果是字符串，直接提取
                OLD_CLIENT_SECRET=$(jq -r '.channels.intclaw.clientSecret // empty' "$SOURCE_CONFIG")
            elif [ "$SECRET_TYPE" = "object" ]; then
                # 如果是对象（SecretInput 引用），保持原样
                OLD_CLIENT_SECRET=$(jq -c '.channels.intclaw.clientSecret' "$SOURCE_CONFIG")
            else
                OLD_CLIENT_SECRET=""
            fi
            ...
            echo "📋 正在迁移配置到 intclaw-connector..."
            ...
            if [ "$SECRET_TYPE" = "string" ]; then
                ...
                if ! jq --arg clientId "$OLD_CLIENT_ID" \
                       ...
                       '.channels."intclaw-connector" = (.channels."intclaw-connector" // {}) | 
                        .channels."intclaw-connector".enabled = true |
                        .channels."intclaw-connector".clientId = $clientId |
                        .channels."intclaw-connector".clientSecret = $clientSecret |
                        .channels.intclaw.enabled = false' \
            ...
            if ! mv "$TEMP_FILE" "$CONFIG_FILE"; then
                ...
                echo "   - 旧 intclaw 插件已禁用"
            else
                echo "ℹ️  未发现旧的 intclaw 配置"
                echo ""
                echo "📝 检测到您是首次配置IntClaw连接器，安装完成后请运行以下命令进行配置："
...
echo "📥 克隆分支: $BRANCH..."
cd /tmp
rm -rf intclaw-openclaw-connector-beta

if ! git clone --single-branch --branch "$BRANCH" \
    https://github.com/IntClaw-Real-AI/intclaw-openclaw-connector.git \
    intclaw-openclaw-connector-beta; then
    echo "❌ 错误：克隆分支 '$BRANCH' 失败"
    echo "💡 提示：请检查分支名是否正确"
    exit 1
fi

cd intclaw-openclaw-connector-beta
...
if echo "$INSTALLED_PLUGINS" | grep -q "intclaw-connector"; then
    echo "📦 检测到已安装的 intclaw-connector，正在卸载..."
    # 使用 -f 强制卸载，避免交互式确认
    # 捕获卸载错误，即使失败也继续执行（可能是插件文件已被手动删除）
    UNINSTALL_FAILED=false
    if command -v yes &> /dev/null; then
        yes | openclaw plugins uninstall intclaw-connector 2>&1 || {
            echo "⚠️  卸载过程出现错误（可能插件文件已被手动删除）"
            UNINSTALL_FAILED=true
        }
    else
        echo "y" | openclaw plugins uninstall intclaw-connector 2>&1 || {
            echo "⚠️  卸载过程出现错误（可能插件文件已被手动删除）"
            UNINSTALL_FAILED=true
        }
    fi
    ...
    if [ "$UNINSTALL_FAILED" = true ] && [ "$HAS_JQ" = true ]; then
        echo "🧹 清理配置文件中的残留配置..."
        if [ -f "$CONFIG_FILE" ]; then
            # 先保存 intclaw-connector 配置，用于后续恢复
            SAVED_CONNECTOR_CONFIG=$(mktemp)
            TEMP_FILES+=("$SAVED_CONNECTOR_CONFIG")
            jq '.channels."intclaw-connector" // {}' "$CONFIG_FILE" > "$SAVED_CONNECTOR_CONFIG"
            
            TEMP_CLEANUP=$(mktemp)
            TEMP_FILES+=("$TEMP_CLEANUP")
            
            # 删除 channels.intclaw-connector 和 plugins 相关配置
            if jq 'del(.channels."intclaw-connector") | 
                  del(.plugins.entries."intclaw-connector") |
                  .plugins.allow = (.plugins.allow // [] | map(select(. != "intclaw-connector"))) |
                  .plugins.load.paths = (.plugins.load.paths // [] | map(select(. | contains("intclaw-openclaw-connector") | not)))' \
                  "$CONFIG_FILE" > "$TEMP_CLEANUP"; then
                mv "$TEMP_CLEANUP" "$CONFIG_FILE"
                echo "✅ 配置文件已清理（channels + plugins + load paths）"
                echo "💾 已保存 intclaw-connector 配置，将在安装后恢复"
            else
    ...
    echo "ℹ️  未检测到已安装的 intclaw-connector，跳过卸载"
fi
...
if [ "$UNINSTALL_FAILED" = true ] && [ "$HAS_JQ" = true ] && [ -f "$SAVED_CONNECTOR_CONFIG" ]; then
    ...
    if [ "$SAVED_CONFIG_CONTENT" != "{}" ] && [ "$SAVED_CONFIG_CONTENT" != "null" ]; then
        ...
        if jq --slurpfile saved "$SAVED_CONNECTOR_CONFIG" \
              '.channels."intclaw-connector" = $saved[0]' \
              "$CONFIG_FILE" > "$TEMP_RESTORE"; then
            mv "$TEMP_RESTORE" "$CONFIG_FILE"
            echo "✅ 配置已恢复"
            
            # 显示恢复的配置信息
            RESTORED_CLIENT_ID=$(jq -r '.channels."intclaw-connector".clientId // empty' "$CONFIG_FILE")
...
if openclaw plugins list 2>/dev/null | grep -q "intclaw-connector"; then
...
if [ -f "$CONFIG_FILE" ]; then
    if [ "$HAS_JQ" = true ]; then
        CONNECTOR_ENABLED=$(jq -r '.channels."intclaw-connector".enabled // false' "$CONFIG_FILE")
        CONNECTOR_CLIENT_ID=$(jq -r '.channels."intclaw-connector".clientId // empty' "$CONFIG_FILE")
...
if [ "$PLUGIN_INSTALLED" = true ] && [ "$CONFIG_VALID" = true ] && [ "$GATEWAY_RUNNING" = true ]; then
    echo "🎉 一切就绪！IntClaw连接器已成功安装并配置完成"
else
    echo "⚠️  需要进一步操作："
...
echo "   - 问题反馈：https://github.com/IntClaw-Real-AI/intclaw-openclaw-connector/issues"
echo ""
echo "💡 如需回滚："
echo "   1. 恢复配置：cp $BACKUP_FILE ~/.openclaw/openclaw.json"
echo "   2. 安装旧版本：openclaw plugins install @intclaw-real-ai/intclaw-connector@latest"
echo ""
