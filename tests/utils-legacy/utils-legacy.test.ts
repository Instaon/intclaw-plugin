...
describe("utils-legacy", () => {
  ...
  it("getIntclawConfig and isIntclawConfigured", async () => {
    const { getIntclawConfig, isIntclawConfigured } = await import("../../src/utils/utils-legacy");
    expect(getIntclawConfig({})).toEqual({});
    expect(isIntclawConfigured({})).toBe(false);
    expect(
      isIntclawConfigured({
        channels: { "intclaw-connector": { clientId: "a", clientSecret: "b" } },
      }),
    ).toBe(true);
  });
  ...
  it("buildMediaSystemPrompt returns markdown", async () => {
    ...
    expect(prompt).toContain("INTCLAW_VIDEO");
    expect(prompt).toContain("INTCLAW_AUDIO");
    expect(prompt).toContain("INTCLAW_FILE");
  });
