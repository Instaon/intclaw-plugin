...
import { clearProbeCache, probeIntclaw } from "../../src/probe";

describe("probeIntclaw", () => {
  beforeEach(() => {
    clearProbeCache();
    vi.restoreAllMocks();
  });
  ...
  it("returns error when credentials are missing", async () => {
    const result = await probeIntclaw(undefined);
  ...
  it("returns aborted when abort signal is pre-aborted", async () => {
    ...
    controller.abort();
    const result = await probeIntclaw(
  ...
  it("returns failure when token response has no accessToken", async () => {
    ...
    const result = await probeIntclaw({ clientId: "id", clientSecret: "secret" });
    expect(result).toEqual({
      ok: false,
      clientId: "id",
  ...
  it("returns success and reuses cache", async () => {
    ...
    const first = await probeIntclaw(creds);
    const second = await probeIntclaw(creds);
