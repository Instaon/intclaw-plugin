...
- **processVideoMarkers(content, webhook, config, oapiToken, log)**  
  解析并处理文本中的 `[INTCLAW_VIDEO]{...}[/INTCLAW_VIDEO]` 标记：
...
|------|------|--------------|----------|------|------|
| 1 | 无 token 跳过处理 | `'hello [INTCLAW_VIDEO]{"path":"/tmp/a.mp4"}[/INTCLAW_VIDEO]'` | `null` | 返回原文（含标记），记录 warn | oapiToken 为空直接 bypass |
| 2 | 无标记直接返回 | `'plain text without markers'` | `'token'` | 返回原文 | 无匹配时不做任何修改 |
| 3 | 非法 JSON 标记 | `'text [INTCLAW_VIDEO]{invalid-json}[/INTCLAW_VIDEO]'` | `'token'` | 返回 `'text'`，记录 warn | 解析失败时安全移除标记 |
...
| 序号 | 场景 | 输入 content | 期望 |
|------|------|--------------|------|
| P1 | 多个合法视频标记 | 含 2 个以上 `[INTCLAW_VIDEO]` | 所有标记均被识别并按顺序处理/移除 |
| P2 | JSON 缺少 path 字段 | `'[INTCLAW_VIDEO]{}[/INTCLAW_VIDEO]'` | 不抛异常，记录 warn，并移除标记或原样保留（以实现为准） |
...
- **processVideoMarkers**
  - 从 AI 回复内容中提取 `[INTCLAW_VIDEO]{...}[/INTCLAW_VIDEO]` 片段：
    - 校验视频文件是否存在
...
- **关联辅助函数（在本方案中作为依赖）**
  ...
  - `uploadMediaToIntClaw`：上传视频与封面至IntClaw媒体服务
...
- **输入**
  - `content = "hello [INTCLAW_VIDEO]{\"path\":\"/tmp/a.mp4\"}[/INTCLAW_VIDEO]"`
  - `oapiToken = null`
- **期望**
...
- **输入**
  - `content = "text [INTCLAW_VIDEO]{invalid-json}[/INTCLAW_VIDEO]"`
  - `oapiToken = "token"`
- **期望**
  - 在解析 JSON 失败时：
...
3. **生成封面图**
   - 调用 `extractVideoThumbnail`，生成的本地路径随后交给 `uploadMediaToIntClaw`
4. **上传视频与封面**
