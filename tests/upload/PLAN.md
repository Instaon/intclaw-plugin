...
- **uploadMediaToIntClaw(filePath, mediaType, accessToken, maxSize, log?)**：根据文件类型与大小上传到IntClaw，成功返回 `media_id`，失败/异常返回 null。
...
- **processVideoMarkers / processFileMarkers / processAudioMarkers**：处理并移除IntClaw媒体标记并执行上传/发送：
...
| 序号 | 场景 | 输入 | 期望 |
|------|------|------|------|
| 16 | video marker（文件不存在） | `hello [INTCLAW_VIDEO]{"path":"/tmp/video.mp4"}[/INTCLAW_VIDEO] world` | 标记被移除；返回内容包含原文本并给出告警/提示 |
| 17 | file marker（存在文件） | `[INTCLAW_FILE]{"path":"/tmp/file.pdf","fileName":"file.pdf","fileType":"pdf"}[/INTCLAW_FILE]` | 标记被移除；触发 `media/upload&type=file` 并发送文件消息 |
| 18 | audio marker（存在文件） | `[INTCLAW_AUDIO]{"path":"/tmp/audio.mp3"}[/INTCLAW_AUDIO]` | 标记被移除；触发 `media/upload&type=voice` 并发送语音消息 |
