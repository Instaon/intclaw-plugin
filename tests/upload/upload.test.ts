...
describe('upload functionality', () => {
  ...
  describe('uploadMediaToIntClaw', () => {
    it('should upload media successfully', async () => {
      const { __testables } = await import('../../test');
      const { uploadMediaToIntClaw } = __testables as any;
      ...
      const result = await uploadMediaToIntClaw('/tmp/file.png', 'image', 'token123', 20 * 1024 * 1024, log);
      ...
      expect(url).toContain('https://oapi.intclaw.com/media/upload?access_token=token123');
    ...
    it('should support different media types', async () => {
      ...
      await uploadMediaToIntClaw('/tmp/file.pdf', 'file', 'token123', 20 * 1024 * 1024, log);
      await uploadMediaToIntClaw('/tmp/video.mp4', 'video', 'token123', 20 * 1024 * 1024, log);
      await uploadMediaToIntClaw('/tmp/audio.mp3', 'voice', 'token123', 20 * 1024 * 1024, log);
  ...
  describe('processVideoMarkers', () => {
    it('should process video markers', async () => {
      ...
      const fs = await import('fs');
      // 用“不存在文件”的分支来验证：能正确识别并移除 INTCLAW_VIDEO 标记
      vi.mocked(fs.existsSync).mockReturnValue(false);

      const content = 'hello\n\n[INTCLAW_VIDEO]{"path":"/tmp/video.mp4"}[/INTCLAW_VIDEO]\n\nworld';
      const result = await processVideoMarkers(content, 'https://webhook.example', {}, 'token123', log);

      expect(result).toContain('hello');
      expect(result).toContain('world');
      expect(result).not.toContain('[INTCLAW_VIDEO]');
  ...
  describe('processFileMarkers', () => {
    it('should process file markers', async () => {
      ...
      mockAxiosPost.mockImplementation((url: string) => {
        if (String(url).includes('https://oapi.intclaw.com/media/upload')) {
      ...
      const content =
        'prefix\n' +
        '[INTCLAW_FILE]{"path":"/tmp/file.pdf","fileName":"file.pdf","fileType":"pdf"}[/INTCLAW_FILE]\n' +
        'suffix';
      const result = await processFileMarkers(content, 'https://webhook.example', {}, 'token123', log);

      expect(result).toContain('prefix');
      expect(result).toContain('suffix');
      expect(result).not.toContain('[INTCLAW_FILE]');
      expect(
  ...
  describe('processAudioMarkers', () => {
    it('should process audio markers', async () => {
      ...
      const content = '[INTCLAW_AUDIO]{"path":"/tmp/audio.mp3"}[/INTCLAW_AUDIO]';
      const result = await processAudioMarkers(content, 'https://webhook.example', {}, 'token123', log);

      expect(result).not.toContain('[INTCLAW_AUDIO]');
      expect(
