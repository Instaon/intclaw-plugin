...
vi.mock("../../src/utils/utils-legacy.ts", () => ({
  ...
  INTCLAW_API: "https://api.intclaw.com",
  INTCLAW_OAPI: "https://oapi.intclaw.com",
...
vi.mock("../../src/services/media/index.ts", () => ({
  ...
  uploadMediaToIntClaw: vi.fn(async () => null),
  toLocalPath: vi.fn((s: string) => s),
  FILE_MARKER_PATTERN: /\[INTCLAW_FILE\](.*?)\[\/INTCLAW_FILE\]/gs,
  VIDEO_MARKER_PATTERN: /\[INTCLAW_VIDEO\](.*?)\[\/INTCLAW_VIDEO\]/gs,
  AUDIO_MARKER_PATTERN: /\[INTCLAW_AUDIO\](.*?)\[\/INTCLAW_AUDIO\]/gs,
}));
...
vi.mock("../../src/reply-dispatcher.ts", () => ({
  createIntclawReplyDispatcher: vi.fn(() => ({
    dispatcher: {},
    replyOptions: {},
...
describe("handleIntClawMessage policy guards", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  async function callHandle(params: {
    ...
    const { handleIntClawMessage } = await import("../../src/core/message-handler");
    await handleIntClawMessage({
      accountId: "acc-1",
