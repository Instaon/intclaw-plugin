...
|------|----------------|------|------|
| 6 | `[{ agentId: 'a', match: { channel: 'other' } }]` | defaultAgentId | 非 intclaw-connector 被筛掉 |
| 7 | `[{ agentId: 'a', match: { channel: 'intclaw-connector' } }]` | 能匹配到 'a'（见优先级5） | channel 匹配 |
...
| 序号 | bindings | 期望 | 说明 |
|------|----------|------|------|
| 22 | `[{ agentId: 'channelOnly', match: { channel: 'intclaw-connector' } }]` | 'channelOnly' | 无 peer 无 accountId 时取第一个 |
