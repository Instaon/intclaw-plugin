...
- **buildDeliverBody(cardInstanceId: string, target: AICardTarget, robotCode: string): any**
  - 职责：构建IntClaw AI Card 投放请求体。target 为 `{ type: 'user', userId }` 或 `{ type: 'group', openConversationId }`，分别生成单聊/群聊的 openSpaceId 与 deliver 模型。
- **buildMsgPayload(msgType: IntClawMsgType, content: string, title?: string): { msgKey, msgParam } | { error }**
