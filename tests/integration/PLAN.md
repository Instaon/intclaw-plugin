...
该套件用于验证插件在**真实IntClaw环境**下的端到端链路（鉴权、发送消息、AI Card 创建/更新/结束、媒体上传等）。它与单元测试不同：依赖真实凭证与外部网络，因此默认会被跳过。

## 2. 运行前置条件

- **环境变量**
  - `INTCLAW_CLIENT_ID`：IntClaw应用 clientId
  - `INTCLAW_CLIENT_SECRET`：IntClaw应用 clientSecret
  - `INTCLAW_TEST_USER_ID`：用于接收消息的测试用户（可选，未设置时会用占位值导致相关用例可能失败/无效）
...
当未配置 `INTCLAW_CLIENT_ID` 或 `INTCLAW_CLIENT_SECRET` 时，测试会被 `describe.skipIf(...)` 跳过，仅保留一个“说明性测试”用于提示需要哪些环境变量（实现位于 `integration.test.ts` 顶部，通过 `plugin.__testables` 调用真实函数）。
...
- **配额/限流**：IntClaw接口可能限流；相关用例仅保证“不抛异常”，不保证每条都成功。
