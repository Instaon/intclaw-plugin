...
/**
 * Integration tests for IntClaw OpenClaw Connector
 * 
 * These tests verify the end-to-end flow of the test.
 * They require actual IntClaw API credentials and should be run
 * in a controlled environment.
 * 
 * To run integration tests:
 * 1. Set INTCLAW_CLIENT_ID and INTCLAW_CLIENT_SECRET environment variables
 * 2. Set INTCLAW_TEST_USER_ID for user-specific tests
 * 3. Run: npm run test:integration
 */

// Skip all tests if integration test environment is not set up
const skipIntegration = !process.env.INTCLAW_CLIENT_ID || !process.env.INTCLAW_CLIENT_SECRET;
...
describe.skipIf(skipIntegration)('Integration Tests', () => {
  ...
  beforeEach(() => {
    config = {
      clientId: process.env.INTCLAW_CLIENT_ID!,
      clientSecret: process.env.INTCLAW_CLIENT_SECRET!,
    };
    testUserId = process.env.INTCLAW_TEST_USER_ID || 'test-user';
  });
  ...
  describe('Media Upload', () => {
    it('should upload image file', async () => {
      const { __testables } = await import('../../test');
      const { uploadMediaToIntClaw, getOapiAccessToken } = __testables as any;
      ...
      try {
        const result = await uploadMediaToIntClaw(testImagePath, 'image', token, 20 * 1024 * 1024);
...
describe('Integration Test Setup', () => {
  it('should have required environment variables for integration tests', () => {
    ...
    if (skipIntegration) {
      ...
      console.log('  INTCLAW_CLIENT_ID=your_client_id');
      console.log('  INTCLAW_CLIENT_SECRET=your_client_secret');
      console.log('  INTCLAW_TEST_USER_ID=your_test_user_id (optional)');
    }
