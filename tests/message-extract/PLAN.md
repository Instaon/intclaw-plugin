...
- **extractMessageContent(data: any): ExtractedMessage**
  - 职责：从IntClaw Stream 回调的 `data` 中根据 `msgtype` 提取统一结构 `ExtractedMessage`，供后续会话、Gateway 等使用。
  - 输入：IntClaw消息体（含 `msgtype`、`text`、`content` 等）。
  - 输出：`{ text, messageType, imageUrls, downloadCodes, fileNames, atIntclawIds, atMobiles }`。

## 2. 分支与用例矩阵（全覆盖）

### 2.1 msgtype 缺失或非预期

| 序号 | 输入 data | 期望 messageType | 期望 text | 期望 imageUrls | 期望 downloadCodes | 期望 fileNames | 期望 atIntclawIds | 期望 atMobiles | 说明 |
|------|-----------|------------------|-----------|----------------|--------------------|----------------|--------------------|----------------|------|
...
| 序号 | 输入 data | 期望 text | 期望 atIntclawIds | 期望 atMobiles | 说明 |
|------|-----------|-----------|--------------------|----------------|------|
...
| 9 | `{ msgtype: 'text', text: { content: 'hi', at: { atIntclawIds: ['id1'], atMobiles: ['13800138000'] } } }` | `'hi'` | `['id1']` | `['13800138000']` | at 完整 |
| 10 | `{ msgtype: 'text', text: { content: 'x', at: { atIntclawIds: ['a','b'], atMobiles: [] } } }` | `'x'` | `['a','b']` | `[]` | 多个 at 用户 |
