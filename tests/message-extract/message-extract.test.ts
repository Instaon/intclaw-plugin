...
describe('extractMessageContent', () => {
  describe('msgtype 缺失或 default 分支', () => {
    it.each([
      ...
      expect(out.atIntclawIds).toEqual([]);
  ...
  describe("msgtype === 'text'", () => {
    it.each([
      { data: { msgtype: 'text' }, text: '', atIntclawIds: [], atMobiles: [] },
      { data: { msgtype: 'text', text: {} }, text: '', atIntclawIds: [], atMobiles: [] },
      { data: { msgtype: 'text', text: { content: ' hello ' } }, text: 'hello', atIntclawIds: [], atMobiles: [] },
      { data: { msgtype: 'text', text: { content: 'hi' } }, text: 'hi', atIntclawIds: [], atMobiles: [] },
      {
        data: {
          msgtype: 'text',
          text: { content: 'hi', at: { atIntclawIds: ['id1'], atMobiles: ['13800138000'] } },
        },
        text: 'hi',
        atIntclawIds: ['id1'],
      ...
      },
      {
        data: {
          msgtype: 'text',
          text: { content: 'x', at: { atIntclawIds: ['a', 'b'], atMobiles: [] } },
        },
        text: 'x',
        atIntclawIds: ['a', 'b'],
        atMobiles: [],
      },
      { data: { msgtype: 'text', text: { content: '' } }, text: '', atIntclawIds: [], atMobiles: [] },
    ] as const)('text=$text atIntclawIds=$atIntclawIds atMobiles=$atMobiles', ({ data, text, atIntclawIds, atMobiles }) => {
      const out = extractMessageContent(data);
      expect(out.messageType).toBe('text');
      expect(out.text).toBe(text);
      expect(out.atIntclawIds).toEqual(atIntclawIds);
