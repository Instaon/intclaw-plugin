...
describe('Gateway Methods - 注册', () => {
  it('应该注册所有方法', () => {
    ...
    // 验证所有方法都已注册
    expect(handlers.has('intclaw-connector.sendToUser')).toBe(true);
    expect(handlers.has('intclaw-connector.sendToGroup')).toBe(true);
    expect(handlers.has('intclaw-connector.send')).toBe(true);
    expect(handlers.has('intclaw-connector.docs.read')).toBe(true);
    expect(handlers.has('intclaw-connector.docs.create')).toBe(true);
    expect(handlers.has('intclaw-connector.docs.append')).toBe(true);
    expect(handlers.has('intclaw-connector.docs.search')).toBe(true);
    expect(handlers.has('intclaw-connector.docs.list')).toBe(true);
    expect(handlers.has('intclaw-connector.status')).toBe(true);
    expect(handlers.has('intclaw-connector.probe')).toBe(true);
...
describe('Gateway Methods - 参数验证', () => {
  ...
  it('sendToUser 缺少 userId 应该返回错误', async () => {
    const { ok, result } = await mockApi.callMethod('intclaw-connector.sendToUser', {
      content: '测试消息',
    });
  ...
  it('sendToGroup 缺少 openConversationId 应该返回错误', async () => {
    const { ok, result } = await mockApi.callMethod('intclaw-connector.sendToGroup', {
      content: '测试消息',
    });
  ...
  it('send 缺少 target 应该返回错误', async () => {
    const { ok, result } = await mockApi.callMethod('intclaw-connector.send', {
      content: '测试消息',
    });
  ...
  it('docs.read 缺少 docId 应该返回错误', async () => {
    const { ok, result } = await mockApi.callMethod('intclaw-connector.docs.read', {
  ...
  it('docs.create 缺少 spaceId 应该返回错误', async () => {
    const { ok, result } = await mockApi.callMethod('intclaw-connector.docs.create', {
      title: '测试文档',
    });
  ...
  it('docs.append 缺少 docId 应该返回错误', async () => {
    const { ok, result } = await mockApi.callMethod('intclaw-connector.docs.append', {
      content: '测试内容',
    });
  ...
  it('docs.search 缺少 keyword 应该返回错误', async () => {
    const { ok, result } = await mockApi.callMethod('intclaw-connector.docs.search', {});
  ...
  it('docs.list 缺少 spaceId 应该返回错误', async () => {
    const { ok, result } = await mockApi.callMethod('intclaw-connector.docs.list', {});
...
describe('Gateway Methods - 状态检查', () => {
  ...
  it('status 应该返回配置状态', async () => {
    const { ok, result } = await mockApi.callMethod('intclaw-connector.status');
...
describe('Gateway Methods - 配置读取', () => {
  it('应该能从 context.deps.getConfig() 获取配置', async () => {
    ...
    const handler = handlers.get('intclaw-connector.status');
