...
vi.mock("../../src/services/messaging/index.ts", () => ({
  sendTextToIntClaw: mockSendText,
  sendMediaToIntClaw: mockSendMedia,
}));

vi.mock("../../src/probe.ts", () => ({
  probeIntclaw: mockProbe,
}));

vi.mock("../../src/config/accounts.ts", () => ({
  resolveIntclawAccount: mockResolveAccount,
  resolveIntclawCredentials: vi.fn(),
  listIntclawAccountIds: vi.fn().mockReturnValue(["main"]),
  resolveDefaultIntclawAccountId: vi.fn().mockReturnValue("main"),
}));

vi.mock("../../src/core/provider.ts", () => ({
  monitorIntclawProvider: mockMonitorProvider,
}));
...
describe("channel plugin", () => {
  ...
  it("loads plugin metadata and base capabilities", async () => {
    const { intclawPlugin } = await import("../../src/channel");
    const plugin = intclawPlugin as any;
    expect(plugin.id).toBe("intclaw-connector");
    expect(plugin.capabilities.media).toBe(true);
    expect(plugin.meta.label).toBe("IntClaw");
  });

  it("config helpers update account flags", async () => {
    const { intclawPlugin } = await import("../../src/channel");
    const plugin = intclawPlugin as any;
    const cfg = { channels: { "intclaw-connector": { accounts: { a1: {} } } } } as any;
    ...
    const updatedDefaultChannel = updatedDefault.channels["intclaw-connector"] as any;
    ...
    expect(updatedNamed.channels["intclaw-connector"].accounts.a1.enabled).toBe(true);
  });
  ...
  it("status.probeAccount delegates to probeIntclaw", async () => {
    const { intclawPlugin } = await import("../../src/channel");
    const plugin = intclawPlugin as any;
