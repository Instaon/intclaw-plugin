...
const mockGetIntclawRuntime = vi.hoisted(() => vi.fn());
...
vi.mock("../../src/runtime.ts", () => ({
  getIntclawRuntime: mockGetIntclawRuntime,
}));
...
describe("reply-dispatcher", () => {
  beforeEach(() => {
    ...
    mockGetIntclawRuntime.mockReturnValue({
      channel: {
        text: {
  ...
  it("creates dispatcher and runs streaming lifecycle callbacks", async () => {
    const { createIntclawReplyDispatcher } = await import("../../src/reply-dispatcher");
    const runtime = {
      log: vi.fn(),
      info: vi.fn(),
      warn: vi.fn(),
    ...
    const result = createIntclawReplyDispatcher({
      cfg: {} as any,
      agentId: "a1",
