...
- **getConfig(cfg: ClawdbotConfig)**  
  返回 `cfg?.channels?.['intclaw-connector'] ?? {}`。
- **isConfigured(cfg)**  
  返回 `Boolean(config.clientId && config.clientSecret)`，config 来自 getConfig。
- **getAccessToken(config)**  
  请求IntClaw OAuth 接口，缓存 token，过期前 60 秒内复用。
...
| 序号 | 输入 cfg | 期望 | 说明 |
|------|----------|------|------|
| 1 | `{ channels: { 'intclaw-connector': { clientId: 'a', clientSecret: 'b' } } }` | 该对象 | 正常 |
| 2 | `{ channels: {} }` | `{}` | 无 intclaw 键 |
| 3 | `{}` | `{}` | 无 channels |
| 4 | `undefined` | `{}` | cfg 缺失 |
...
| 9 | `{ channels: { 'intclaw-connector': { clientId: 'a' } } }` | false | 缺 clientSecret |
| 10 | `{ channels: { 'intclaw-connector': { clientSecret: 'b' } } }` | false | 缺 clientId |
...
|------|----------|------|------|
| P1 | `null` | `{}` | cfg 为 null |
| P2 | `{ channels: { 'intclaw-connector': null } }` | 视实现：`null` 或 `{}` | 键存在但值为 null |
