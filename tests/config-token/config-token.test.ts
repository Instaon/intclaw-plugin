...
describe('config & token helpers', () => {
  ...
  describe('getConfig / isConfigured', () => {
    it('should extract intclaw-connector config from ClawdbotConfig', async () => {
      const { __testables } = await import('../../test');
      const { getConfig } = __testables as any;
      const cfg = getConfig(baseCfg);
      expect(cfg).toEqual(baseCfg.channels['intclaw-connector']);
    });
    ...
    it('should consider config valid only when clientId and clientSecret exist', async () => {
      ...
      expect(
        isConfigured({
          channels: { 'intclaw-connector': { clientId: 'id-only' } },
        } as any),
      ).toBe(false);
      expect(
        isConfigured({
          channels: { 'intclaw-connector': { clientSecret: 'secret-only' } },
        } as any),
      ).toBe(false);
    });
  });
  ...
  describe('getAccessToken', () => {
    it('should request new token and cache it', async () => {
      ...
      const token1 = await getAccessToken(baseCfg.channels['intclaw-connector']);
      const token2 = await getAccessToken(baseCfg.channels['intclaw-connector']);
  ...
  describe('getOapiAccessToken', () => {
    it('should return token when oapi returns success', async () => {
      ...
      const token = await getOapiAccessToken(baseCfg.channels['intclaw-connector']);
  ...
  describe('getUnionId', () => {
    it('should call oapi once and then use cache', async () => {
      ...
      const cfg = baseCfg.channels['intclaw-connector'];
